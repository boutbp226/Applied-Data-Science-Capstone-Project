...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        filtered_df =filtered_df.groupby(['Launch Site', 'class']).size().reset_index(name='outcome_class_count')
        fig=px.pie(filtered_df, values='outcome_class_count', names='class',title=f'Total Success Launches for Site {entered_site}')
        return fig
# TASK 4:
...
def get_scatter_chart(entered_site,Payload_Mass):
    ...
    if entered_site == 'ALL':
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', title='Correlation between Payload and Success for All Sites')
        return fig
    else:
